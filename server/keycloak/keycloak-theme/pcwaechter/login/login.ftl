          ...
          <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
            ...
            <div class="kc-form-footer">
              <#if realm.resetPasswordAllowed>
                <a tabindex="5" href="${url.loginResetCredentialsUrl}" class="kc-link" onclick="return showForgotPasswordPanel(event)">Passwort vergessen?</a>
              </#if>
              <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
                <a tabindex="6" href="${url.registrationUrl}" class="kc-link" onclick="return showRegisterPanel(event)">Registrieren</a>
              </#if>
            </div>
          </form>
