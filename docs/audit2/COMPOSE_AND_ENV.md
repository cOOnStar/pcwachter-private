...
| `exports_data` | API Export-Verzeichnis | `server/infra/compose/docker-compose.yml:13`, `126-128` |
| `uploads_data` | API Upload-Verzeichnis | `server/infra/compose/docker-compose.yml:14`, `126-128` |
...
| `api` | host `18080:8000` | `pcw-internal` | `exports_data`, `uploads_data` | `server/infra/compose/docker-compose.yml:93-141`, `126-130`, `134-135` |
| `console` | host `13000:80` | `pcw-internal` | none | `server/infra/compose/docker-compose.yml:146-170`, `163-164` |
| `home` | host `13001:3000` | `pcw-internal` | none | `server/infra/compose/docker-compose.yml:174-205`, `199-200` |
| `zammad-*` (profile `zammad`) | `zammad-nginx` host `3001:3001` | `zammad-internal` (+ `pcw-internal` bei nginx) | mehrere `zammad-*` Volumes | `server/infra/compose/docker-compose.yml:212-408` |
...
| `PCW_KEYCLOAK_IMAGE`, `PCW_API_IMAGE`, `PCW_CONSOLE_IMAGE`, `PCW_HOME_IMAGE` | `.env.example:11-14`, `server/infra/compose/docker-compose.yml:49`, `94`, `147`, `175` |
...
| Variable | Quelle(n) |
|---|---|
| `DATABASE_URL` | `server/api/app/settings.py:5`, `server/infra/compose/docker-compose.yml:101-102` |
| `API_KEYS`, `AGENT_API_KEYS`, `RATELIMIT_REDIS_URL` | `server/api/app/settings.py:10-12`, `.env.example:45-47` |
| `AGENT_BOOTSTRAP_KEY`, `ALLOW_LEGACY_API_KEY_REGISTER` | `server/api/app/settings.py:16-19` |
| `KEYCLOAK_URL`, `KEYCLOAK_REALM`, `KEYCLOAK_AUDIENCE`, `KEYCLOAK_ISSUER` | `server/api/app/settings.py:21-24`, `.env.example:35-40`, `server/infra/compose/docker-compose.yml:105-113` |
| `KEYCLOAK_ADMIN_CLIENT_ID`, `KEYCLOAK_ADMIN_CLIENT_SECRET`, `KEYCLOAK_ADMIN_USER`, `KEYCLOAK_ADMIN_PASSWORD` | `server/api/app/settings.py:25-29`, `.env.example:37-38`, `server/infra/compose/docker-compose.yml:107-110` |
| `CONSOLE_ALLOWED_ROLES`, `CORS_ORIGINS` | `server/api/app/settings.py:31-35`, `.env.example:48-49`, `server/infra/compose/docker-compose.yml:111-113` |

### Home / Console
| Variable | Quelle(n) |
|---|---|
| `VITE_API_BASE_URL`, `VITE_KEYCLOAK_URL`, `VITE_KEYCLOAK_REALM`, `VITE_KEYCLOAK_CLIENT_ID` | `.env.example:55-58`, `server/infra/compose/docker-compose.yml:152-155`, `server/console/Dockerfile:8-15` |
| `NEXT_PUBLIC_API_URL`, `AUTH_KEYCLOAK_ID`, `AUTH_KEYCLOAK_SECRET`, `AUTH_KEYCLOAK_ISSUER`, `AUTH_SECRET`, `API_INTERNAL_URL` | `.env.example:63-70`, `server/infra/compose/docker-compose.yml:185-190`, `server/home/src/auth.ts:7-10` |

### Optional Integrationen
| Variable | Quelle(n) |
|---|---|
| `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PUBLISHABLE_KEY` | `.env.example:75-77`, `server/api/app/settings.py:54-56`, `server/infra/compose/docker-compose.yml:117-119` |
| `ZAMMAD_BASE_URL`, `ZAMMAD_API_TOKEN`, `ZAMMAD_WEBHOOK_SECRET`, `ZAMMAD_DEFAULT_GROUP_ID`, `ZAMMAD_DEFAULT_ORG_ID`, `ZAMMAD_CUSTOMER_ROLE_ID` | `.env.example:81-83`, `server/api/app/settings.py:38-45`, `server/infra/compose/docker-compose.yml:114-115` |
| `ZAMMAD_IMAGE`, `ZAMMAD_POSTGRES_DB`, `ZAMMAD_POSTGRES_USER`, `ZAMMAD_POSTGRES_PASSWORD`, `ZAMMAD_HOSTNAME` | `.env.example:90-94`, `server/infra/compose/docker-compose.yml:273`, `258-260`, `389-390` |
| `FIGMA_PREVIEW_KEY` | `.env.example:86`, `server/api/app/settings.py:51`, `server/infra/compose/docker-compose.yml:120` |
